...
| — | ✅ **OSD-352 RR-3 brain Visium** 발견 (3+3) | 새로운 primary spatial dataset |
...
| OSD-352 brain Visium | ✅ 확인 (Masarapu et al., *Nat. Comm.* 2024, RR-3, 3+3) | ✅ 완료 |
...
| Phase 2 (공간) | OSD-352 Visium 데이터 접근 가능 | GLDS-270 heart만으로 축소 (exploratory only) |
...
### 검증 기준
- [x] ≥2 종 쌍에서 NES 상관 유의미 (perm p < 0.05): Dros↔skin r=−0.586***, Dros↔thymus r=−0.188**, eye↔liver r=+0.324*** 등 다수
- [x] 각 쌍별 ortholog 커버리지 + pathway 커버리지 보고: Drosophila 222/294 KEGG, 31.2% ortholog coverage
- [x] ~~예상: mouse-Drosophila > mouse-Arabidopsis~~ → **결과**: Drosophila↔mouse 유의미하나 **음의 상관** (반보존). Arabidopsis cross-kingdom ortholog 0개 → E4b 불가

### 산출물
- `v3/data/mouse_human_orthologs.tsv`, `v3/data/ensmusg_to_symbol.tsv`
- `v3/evaluation/E4_multispecies_nes.json`, `E5_phylogenetic.json`
...
| 데이터셋 | 조직 | 미션 | 기술 | n | 비고 |
|----------|------|------|------|---|------|
| **OSD-352 brain** | Brain (cortex, hippocampus, striatum, thalamus, hypothalamus) | RR-3 | 10x Visium | 3 FLT + 3 GC, female BALB/c | ★ Primary (Masarapu et al., *Nat. Comm.* 2024) |
...
**F3: OSD-352 RR-3 Brain Visium** (Primary)
...
| F3c | Region analysis | 뇌 해부학적 영역별 (cortex, hippocampus, striatum, thalamus) 반응 차이 |
...
### 검증 기준
- [x] ~~F3a: Spot-level AUROC > 0.6~~ → **FAILED**: Section AUROC=0.139, Animal AUROC=0.444. Brain is a genuine negative tissue
- [x] ~~F3b: ≥20 SVGs FDR < 0.05~~ → **FAILED**: SVG count difference NS (p=0.240). Jaccard=0.805 (anatomy dominates)
- [x] F3d: Spot-level vs bulk AUROC 비교 보고 완료: spatial 0.444 vs bulk 0.000 (n=6 overfitting)

### 산출물
- `v3/evaluation/F3a_visium_classification.json`, `F3b_visium_svg.json`, `F3d_visium_cross_resolution.json`
...
### 검증 기준
- [x] ~~F5c: ≥3 cell types AUROC > 0.7~~ → **PARTIAL**: 2/3 달성 (NK 0.845, T 0.752). CD4 0.651 근접
- [x] ~~F5d: RRRM-1↔RRRM-2 r > 0.352~~ → **FAILED**: T cell r=+0.011, max erythroid r=−0.362. Cross-mission concordance 낮음
- [x] F5e: Bone marrow 분류 불가 확인. ALL 14 cell types near chance (0.27-0.54)

### 산출물
- `v3/evaluation/F5A_rrrm2_composition.json`, `F5B_rrrm2_fgsea.json`, `F5C_rrrm2_loao.json`, `F5D_rrrm2_cross_mission.json`, `F5E_rrrm2_bone_marrow.json`
...
|------|------|-----------|---------|
| PCA-LR | Classical | 6-tissue mean 0.758 (liver 0.670, thymus 0.923) | 신규 조직에도 적용 |
...
- **FM 결론**: PCA-LR (liver 0.670, 6-tissue mean 0.758) 대비 모든 FM이 열등. scFoundation이 UCE보다 전반적으로 우수. 유의미: scFoundation liver (p=0.001), gastro (p=0.035); UCE thymus (p=0.031). FM은 spaceflight detection에 PCA-LR 대비 이점 없음 → 사전학습 cell atlas 지식이 spaceflight perturbation 탐지에 도움 안 됨.
...
### 산출물
- `v3/evaluation/A7_lung.json`, `A7b_colon.json`, `A8_extended_skin.json`, `B_ext_transfer_matrix.json`
...
### 검증 기준
- [x] R1: Radiation main effect 분리 완료. Skin rad AUROC=0.762*, HLU 0.714*
- [x] R2: ISS↔radiation concordance 보고 완료. radiation→ISS r=+0.14~+0.44; HLU→ISS r=−0.20~−0.50
- [x] R3: Combined ≈ 0 (radiation + HLU 상쇄). Radiation alone이 ISS와 더 유사
...
| v3_Fig2 | Spatial Visium: brain spot-level classification + SVG overlap | Phase 2 |
| v3_Fig3 | RRRM-2 scRNA: cell-type benchmarks + RRRM-1 vs RRRM-2 concordance | Phase 3 |
| v3_Fig4 | 기존 확장: 신규 조직 A7/A7b/A8 + radiation R1-R3 | Phase 4+5 |
| v3_Fig5 | FM 비교: PCA-LR vs Geneformer/scGPT/UCE/scFoundation × 7 tissues | Phase 4 |
...
| OSD-352 Visium Seurat RDS 손상 | 중 | Phase 2 메타데이터 부분 손실 | 데이터 기반 section mapping 유도 (완료) |
