#!/usr/bin/env python3
"""Build visual QA crops for slide 43 v8 transition asset."""
...
SOURCE = ASSET_DIR / "v8_starts_after_the_evidence_gates_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"v8_transition_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"v8_transition_{name}.png"
    ...
    (target_dir / "v8_transition_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "02_title_subtitle", "box": (120, 310, 3300, 520)},
        {"name": "03_evidence_gates", "box": (120, 610, 1445, 1438)},
        {"name": "04_threshold_arrows", "box": (1440, 650, 2028, 1410)},
        {"name": "05_v8_modules", "box": (2025, 610, 3720, 1438)},
        {"name": "06_reader_move", "box": (120, 1500, 3720, 1818)},
    ...
    strict_specs = [
        ...
        {"name": "03_gate_rows_top", "box": (120, 610, 1445, 1040)},
        {"name": "04_gate_rows_bottom", "box": (120, 1000, 1445, 1438)},
        {"name": "05_threshold_center", "box": (1485, 650, 1940, 1410)},
        {"name": "06_arrow_clearance", "box": (1370, 890, 2090, 1135)},
        {"name": "07_module_bridge_decompose", "box": (2025, 610, 3720, 1020)},
        {"name": "08_module_intervene_causal", "box": (2025, 990, 3720, 1438)},
        {"name": "09_reader_nodes", "box": (500, 1635, 3670, 1818)},
        {"name": "10_footer", "box": (120, 1910, 3720, 2076)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "v8_transition_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "v8_transition_STRICT_QA_contact_sheet.png")
