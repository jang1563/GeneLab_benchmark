#!/usr/bin/env python3
"""Run diagnostic-only donor holdouts for the v9 human organoid pilot."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        default="v9/human_organoid/reports/donor_diagnostics",
        help="Directory for diagnostic predictions, metrics, run manifest, and summary.",
    )
    parser.add_argument(
        "--top-variable-genes",
        type=int,
        default=2000,
        help="Number of train-fold variable genes to use per diagnostic fold.",
    )
    parser.add_argument(
        "--transform",
        choices=["none", "log1p"],
...
def main() -> None:
    args = parse_args()
    _, summary = run_human_organoid_donor_diagnostics(
