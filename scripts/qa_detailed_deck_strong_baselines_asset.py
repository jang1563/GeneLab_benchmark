#!/usr/bin/env python3
"""Build strict visual QA crops for the strong-baselines asset."""
...
SOURCE = ASSET_DIR / "strong_baselines_make_model_claims_meaningful_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"strong_baselines_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"strong_baselines_{name}.png"
    ...
    (target_dir / "strong_baselines_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "02_baseline_contract", "box": (120, 330, 1160, 1840)},
        {"name": "03_classical_floor", "box": (1160, 330, 2540, 1840)},
        {"name": "04_model_interpretation", "box": (2540, 330, 3730, 1840)},
    ...
    strict_specs = [
        ...
        {"name": "03_contract_top", "box": (150, 355, 1125, 820)},
        {"name": "04_contract_bottom", "box": (150, 800, 1125, 1810)},
        {"name": "05_classical_chart_title_axis", "box": (1210, 355, 2490, 640)},
        {"name": "06_classical_bars_top", "box": (1210, 560, 2490, 1010)},
        {"name": "07_classical_bars_bottom", "box": (1210, 980, 2490, 1410)},
        {"name": "08_result_surface_card", "box": (1210, 1450, 2490, 1795)},
        {"name": "09_model_comparison_bars", "box": (2600, 520, 3670, 1045)},
        {"name": "10_interpretation_rule", "box": (2600, 1160, 3670, 1685)},
        {"name": "11_footer_source", "box": (130, 1860, 3710, 2130)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "strong_baselines_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "strong_baselines_STRICT_QA_contact_sheet.png")
