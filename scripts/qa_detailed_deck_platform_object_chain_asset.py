#!/usr/bin/env python3
"""Build visual QA crops for slide 51 platform object-chain asset."""
...
ASSET_DIR = (
    ...
    / "assets"
    / "platform_object_chain"
)
SOURCE = ASSET_DIR / "manifest_evaluator_run_record_make_scores_rerunnable_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"platform_object_chain_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"platform_object_chain_{name}.png"
    ...
    (target_dir / "platform_object_chain_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "03_main_object_chain", "box": (120, 610, 3720, 1462)},
        {"name": "04_bottom_path", "box": (120, 1530, 3720, 1848)},
        {"name": "05_footer_takeaway", "box": (120, 1910, 3720, 2076)},
    ...
    strict_specs = [
        ...
        {"name": "03_task_manifest_panel", "box": (120, 610, 1210, 1462)},
        {"name": "04_evaluator_panel", "box": (1375, 610, 2465, 1462)},
        {"name": "05_run_record_panel", "box": (2630, 610, 3720, 1462)},
        {"name": "06_between_panel_arrows", "box": (1180, 905, 2660, 1148)},
        {"name": "07_bottom_file_path", "box": (120, 1530, 3720, 1848)},
        {"name": "08_footer", "box": (120, 1910, 3720, 2076)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "platform_object_chain_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "platform_object_chain_STRICT_QA_contact_sheet.png")
