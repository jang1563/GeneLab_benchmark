#!/usr/bin/env python3
"""Build visual QA crops for slide 2 benchmark-need asset."""
...
SOURCE = ASSET_DIR / "why_this_needs_a_benchmark_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"benchmark_need_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"benchmark_need_{name}.png"
    ...
    (target_dir / "benchmark_need_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "02_title_subtitle", "box": (120, 350, 1880, 620)},
        {"name": "03_public_records", "box": (120, 650, 1190, 1645)},
        {"name": "04_benchmark_layer", "box": (1210, 650, 2470, 1645)},
        {"name": "05_hierarchy_map", "box": (2490, 650, 3720, 1645)},
        {"name": "06_flow_and_footer", "box": (130, 1620, 3710, 2095)},
    ...
    strict_specs = [
        ...
        {"name": "03_title_subtitle", "box": (120, 350, 1880, 620)},
        {"name": "04_source_stats", "box": (170, 880, 1125, 1155)},
        {"name": "05_source_stack", "box": (170, 1160, 1125, 1590)},
        {"name": "06_benchmark_steps_top", "box": (1260, 700, 2400, 1135)},
        {"name": "07_benchmark_steps_bottom", "box": (1260, 1110, 2400, 1595)},
        {"name": "08_hierarchy_nodes", "box": (2520, 700, 3690, 1595)},
        {"name": "09_flow_line", "box": (260, 1625, 3560, 1725)},
        {"name": "10_footer_scope", "box": (130, 1870, 3710, 2095)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "benchmark_need_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "benchmark_need_STRICT_QA_contact_sheet.png")
