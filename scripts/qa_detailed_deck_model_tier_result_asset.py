#!/usr/bin/env python3
"""Build strict visual QA crops for the model-tier result asset."""
...
SOURCE = ASSET_DIR / "model_tier_result_task_fit_beats_scale_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"model_tier_result_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"model_tier_result_{name}.png"
    ...
    (target_dir / "model_tier_result_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "02_comparison_rules", "box": (120, 330, 1085, 1840)},
        {"name": "03_shared_readout", "box": (1070, 330, 2490, 1840)},
        {"name": "04_tissue_deltas", "box": (2480, 330, 3730, 1840)},
    ...
    strict_specs = [
        ...
        {"name": "03_rule_cards_top", "box": (165, 360, 1045, 980)},
        {"name": "04_rule_cards_bottom", "box": (165, 920, 1045, 1810)},
        {"name": "05_shared_axis_top", "box": (1140, 370, 2405, 770)},
        {"name": "06_shared_rows_middle", "box": (1140, 700, 2405, 1325)},
        {"name": "07_shared_rows_bottom", "box": (1140, 1250, 2405, 1810)},
        {"name": "08_delta_axis_top", "box": (2555, 370, 3655, 760)},
        {"name": "09_delta_rows_middle", "box": (2555, 700, 3655, 1290)},
        {"name": "10_delta_rows_bottom", "box": (2555, 1230, 3655, 1810)},
        {"name": "11_footer_source", "box": (130, 1860, 3710, 2130)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "model_tier_result_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "model_tier_result_STRICT_QA_contact_sheet.png")
