#!/usr/bin/env python3
"""Build visual QA crops for slide 50 platform turn asset."""
...
SOURCE = ASSET_DIR / "platform_turn_packages_results_into_reproducible_benchmark_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"platform_turn_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"platform_turn_{name}.png"
    ...
    (target_dir / "platform_turn_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "03_main_panels", "box": (120, 610, 3720, 1462)},
        {"name": "04_object_chain", "box": (120, 1516, 3720, 1848)},
    ...
    strict_specs = [
        ...
        {"name": "03_evidence_workstreams", "box": (120, 610, 1345, 1462)},
        {"name": "04_platform_turn", "box": (1415, 610, 2375, 1462)},
        {"name": "05_benchmark_system", "box": (2445, 610, 3720, 1462)},
        {"name": "06_object_chain_arrows", "box": (120, 1516, 3720, 1848)},
        {"name": "07_footer", "box": (120, 1910, 3720, 2076)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "platform_turn_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "platform_turn_STRICT_QA_contact_sheet.png")
