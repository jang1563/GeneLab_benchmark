#!/usr/bin/env python3
"""Build strict visual QA crops for the release-roadmap asset."""
...
SLIDE = ASSET_DIR / "roadmap_to_release_premium.png"
...
def crop_set(target_dir: Path, specs: list[tuple[str, tuple[int, int, int, int]]], filename: str) -> Path:
    ...
    for name, box in specs:
        crop = slide.crop(box)
        out = target_dir / f"release_roadmap_{name}.png"
    ...
    for name, _box in specs:
        crop = Image.open(target_dir / f"release_roadmap_{name}.png")
    ...
    (target_dir / "release_roadmap_crop_manifest.json").write_text(json.dumps(entries, indent=2) + "\n")
    return sheet
...
def main() -> None:
    qa_specs = [
        ...
        ("roadmap_table", (90, 580, 3745, 1495)),
        ("release_runway", (90, 1500, 3745, 1875)),
        ("footer", (90, 1880, 3745, 2105)),
    ]
    strict_specs = [
        ("01_full_thumbnail", (0, 0, 3840, 2160)),
        ("02_header_title_badges", (90, 42, 3745, 520)),
        ("03_table_headers", (435, 650, 3620, 745)),
        ("04_data_package_lane", (150, 755, 3600, 915)),
        ("05_evaluator_lane", (150, 920, 3600, 1085)),
        ("06_extension_lane", (150, 1090, 3600, 1255)),
        ("07_paper_lane", (150, 1255, 3600, 1438)),
        ("08_release_runway", (90, 1560, 3745, 1815)),
        ("09_footer_takeaway", (120, 1910, 3720, 2076)),
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "release_roadmap_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "release_roadmap_STRICT_QA_contact_sheet.png")
