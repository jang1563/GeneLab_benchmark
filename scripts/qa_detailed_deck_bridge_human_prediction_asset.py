#!/usr/bin/env python3
"""Build visual QA crops for slide 45 BRIDGE human-prediction asset."""
...
ASSET_DIR = (
    ...
    / "assets"
    / "bridge_human_prediction"
)
SOURCE = ASSET_DIR / "mouse_pathways_improve_human_prediction_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"bridge_human_prediction_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"bridge_human_prediction_{name}.png"
    ...
    (target_dir / "bridge_human_prediction_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "03_heatmap_panel", "box": (120, 610, 1360, 1460)},
        {"name": "04_auroc_lift_panel", "box": (1415, 610, 2625, 1460)},
        {"name": "05_tissue_contribution_panel", "box": (2680, 610, 3720, 1460)},
        {"name": "06_bridge_flow_strip", "box": (120, 1522, 3720, 1848)},
    ...
    strict_specs = [
        ...
        {"name": "03_heatmap_values", "box": (360, 790, 1265, 1265)},
        {"name": "04_heatmap_callout", "box": (120, 1210, 1360, 1460)},
        {"name": "05_rf_lift", "box": (1475, 790, 2560, 1035)},
        {"name": "06_lr_lift", "box": (1475, 960, 2560, 1210)},
        {"name": "07_primary_readout", "box": (1475, 1180, 2560, 1405)},
        {"name": "08_tissue_bars", "box": (2680, 800, 3720, 1220)},
        {"name": "09_tissue_reader_rule", "box": (2680, 1210, 3720, 1460)},
        {"name": "10_flow_nodes", "box": (520, 1620, 3720, 1848)},
        {"name": "11_footer", "box": (120, 1910, 3720, 2076)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "bridge_human_prediction_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "bridge_human_prediction_STRICT_QA_contact_sheet.png")
