#!/usr/bin/env python3
"""Build visual QA crops for slide 6 model-input-surface asset."""
...
SOURCE = ASSET_DIR / "what_the_model_actually_sees_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"model_input_surface_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"model_input_surface_{name}.png"
    ...
    (target_dir / "model_input_surface_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "02_title_subtitle", "box": (120, 350, 2250, 625)},
        {"name": "03_same_sample_panel", "box": (120, 620, 980, 1608)},
        {"name": "04_feature_lanes", "box": (1000, 620, 2720, 1608)},
        {"name": "05_scoring_gate", "box": (2740, 620, 3735, 1608)},
        {"name": "06_arrows_and_lane_edges", "box": (880, 760, 2840, 1510)},
        {"name": "07_reader_rule", "box": (120, 1600, 3720, 1835)},
        {"name": "08_footer_scope", "box": (120, 1865, 3720, 2095)},
        {"name": "09_full_downscaled", "box": (0, 0, 3840, 2160)},
    ]
    strict_specs = [
        {"name": "01_full_thumbnail", "box": (0, 0, 3840, 2160)},
        {"name": "02_title_subtitle", "box": (120, 350, 2250, 625)},
        {"name": "03_sample_rows_table", "box": (180, 905, 900, 1495)},
        {"name": "04_feature_definition", "box": (180, 1410, 900, 1575)},
        {"name": "05_gene_lane", "box": (1060, 810, 2645, 1100)},
        {"name": "06_pathway_lane", "box": (1060, 1085, 2645, 1375)},
        {"name": "07_compressed_lane", "box": (1060, 1350, 2645, 1580)},
        {"name": "08_right_arrows", "box": (2550, 800, 2880, 1510)},
        {"name": "09_classifier_and_probs", "box": (2835, 865, 3640, 1265)},
        {"name": "10_score_row", "box": (2820, 1240, 3658, 1545)},
        {"name": "11_reader_rule_steps", "box": (1250, 1630, 3650, 1810)},
        {"name": "12_footer", "box": (120, 1865, 3720, 2095)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "model_input_surface_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "model_input_surface_STRICT_QA_contact_sheet.png")
