#!/usr/bin/env python3
"""Build strict visual QA crops for the transfer-feasibility asset."""
...
SOURCE = ASSET_DIR / "screen_transfer_feasibility_before_training_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"transfer_feasibility_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"transfer_feasibility_{name}.png"
    ...
    (target_dir / "transfer_feasibility_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "02_workflow_panel", "box": (130, 330, 1285, 1820)},
        {"name": "03_calibration_table", "box": (1300, 330, 2625, 1820)},
        {"name": "04_triage_board", "box": (2640, 330, 3715, 1820)},
    ...
    strict_specs = [
        ...
        {"name": "03_workflow_top", "box": (180, 350, 1225, 925)},
        {"name": "04_workflow_bottom_reader_rule", "box": (180, 900, 1225, 1790)},
        {"name": "05_calibration_stat_card", "box": (1360, 460, 2545, 665)},
        {"name": "06_calibration_rows_top", "box": (1360, 690, 2545, 1160)},
        {"name": "07_calibration_rows_bottom", "box": (1360, 1135, 2545, 1645)},
        {"name": "08_calibration_use_note", "box": (1360, 1625, 2545, 1790)},
        {"name": "09_triage_board_axis_points", "box": (2700, 500, 3635, 1235)},
        {"name": "10_triage_decision_cards", "box": (2700, 1250, 3635, 1790)},
        {"name": "11_footer_source", "box": (150, 1880, 3690, 2045)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "transfer_feasibility_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "transfer_feasibility_STRICT_QA_contact_sheet.png")
