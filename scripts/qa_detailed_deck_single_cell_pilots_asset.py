#!/usr/bin/env python3
"""Build visual QA crops for slide 35 single-cell pilots asset."""
...
SOURCE = ASSET_DIR / "single_cell_pilots_provide_context_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"single_cell_pilots_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"single_cell_pilots_{name}.png"
    ...
    (target_dir / "single_cell_pilots_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "02_title_subtitle", "box": (120, 350, 2500, 625)},
        {"name": "03_inventory_panel", "box": (120, 610, 1325, 1655)},
        {"name": "04_composition_panel", "box": (1335, 610, 2540, 1655)},
        {"name": "05_extension_panel", "box": (2550, 610, 3755, 1655)},
    ...
    strict_specs = [
        ...
        {"name": "02_title_subtitle", "box": (120, 350, 2500, 625)},
        {"name": "03_inventory_stats", "box": (170, 745, 1260, 985)},
        {"name": "04_inventory_bars", "box": (170, 1110, 1335, 1620)},
        {"name": "05_composition_stats", "box": (1385, 745, 2475, 930)},
        {"name": "06_composition_lollipops", "box": (1385, 1000, 2555, 1545)},
        {"name": "07_extension_pbmc", "box": (2605, 745, 3705, 1420)},
        {"name": "08_extension_bm_cards", "box": (2605, 1460, 3705, 1625)},
        {"name": "09_reader_rule_arrows", "box": (150, 1695, 3690, 1828)},
        {"name": "10_footer", "box": (120, 1865, 3720, 2095)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "single_cell_pilots_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "single_cell_pilots_STRICT_QA_contact_sheet.png")
