#!/usr/bin/env python3
"""Build strict visual QA crops for the tissue hierarchy asset."""
...
SOURCE = ASSET_DIR / "tissue_transfer_hierarchy_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"tissue_hierarchy_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"tissue_hierarchy_{name}.png"
    ...
    (target_dir / "tissue_hierarchy_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "02_reading_guide", "box": (130, 330, 935, 1820)},
        {"name": "03_forest_panel", "box": (930, 330, 2625, 1820)},
        {"name": "04_summary_panel", "box": (2620, 330, 3715, 1820)},
    ...
    strict_specs = [
        ...
        {"name": "03_reading_legend", "box": (150, 520, 910, 1390)},
        {"name": "04_reading_callout", "box": (150, 1480, 910, 1800)},
        {"name": "05_forest_top_rows", "box": (975, 515, 2580, 980)},
        {"name": "06_forest_mid_rows", "box": (975, 930, 2580, 1395)},
        {"name": "07_forest_bottom_axis", "box": (975, 1335, 2580, 1800)},
        {"name": "08_summary_tiers", "box": (2655, 520, 3680, 1120)},
        {"name": "09_pair_consistency", "box": (2655, 1135, 3680, 1495)},
        {"name": "10_carry_forward", "box": (2655, 1485, 3680, 1800)},
        {"name": "11_footer", "box": (150, 1880, 3690, 2045)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "tissue_hierarchy_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "tissue_hierarchy_STRICT_QA_contact_sheet.png")
