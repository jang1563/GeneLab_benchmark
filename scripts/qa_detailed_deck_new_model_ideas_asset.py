#!/usr/bin/env python3
"""Build strict visual QA crops for the v7 new-model-ideas asset."""
...
SOURCE = ASSET_DIR / "newer_model_ideas_preserve_the_lesson_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"new_model_ideas_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"new_model_ideas_{name}.png"
    ...
    (target_dir / "new_model_ideas_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "02_v7_probe_panel", "box": (120, 330, 1075, 1840)},
        {"name": "03_comparison_surface", "box": (1065, 330, 2535, 1840)},
        {"name": "04_decision_readout", "box": (2525, 330, 3730, 1840)},
        {"name": "05_footer_source", "box": (120, 1850, 3730, 2115)},
    ...
    strict_specs = [
        ...
        {"name": "03_probe_top", "box": (150, 500, 1045, 1035)},
        {"name": "04_probe_bottom", "box": (150, 1040, 1045, 1805)},
        {"name": "05_scprint_chart", "box": (1120, 520, 2480, 1075)},
        {"name": "06_gnn_heatmap", "box": (1120, 1130, 2480, 1800)},
        {"name": "07_stats_tiles", "box": (2570, 520, 3690, 745)},
        {"name": "08_mean_bars", "box": (2570, 760, 3690, 1105)},
        {"name": "09_graph_structure", "box": (2570, 1110, 3690, 1445)},
        {"name": "10_takeaway", "box": (2570, 1460, 3690, 1805)},
        {"name": "11_footer_source", "box": (130, 1870, 3710, 2100)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "new_model_ideas_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "new_model_ideas_STRICT_QA_contact_sheet.png")
