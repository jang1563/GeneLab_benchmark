#!/usr/bin/env python3
"""Build visual QA crops for slide 42 prediction-to-hypothesis asset."""
...
SOURCE = ASSET_DIR / "prediction_becomes_hypothesis_through_evidence_gates_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"prediction_hypothesis_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"prediction_hypothesis_{name}.png"
    ...
    (target_dir / "prediction_hypothesis_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "02_title_subtitle", "box": (120, 310, 3280, 510)},
        {"name": "03_claim_ladder", "box": (120, 610, 3720, 1380)},
        {"name": "04_reader_rule", "box": (120, 1435, 3720, 1690)},
        {"name": "05_transition_panel", "box": (120, 1732, 3720, 1858)},
        {"name": "06_footer_scope", "box": (120, 1910, 3720, 2076)},
        {"name": "07_full_downscaled", "box": (0, 0, 3840, 2160)},
    ]
    strict_specs = [
        {"name": "01_full_thumbnail", "box": (0, 0, 3840, 2160)},
        {"name": "02_header_title_badges", "box": (100, 50, 3740, 510)},
        {"name": "03_prediction_card", "box": (120, 610, 950, 1380)},
        {"name": "04_robustness_card", "box": (1005, 610, 1835, 1380)},
        {"name": "05_biology_card", "box": (1890, 610, 2720, 1380)},
        {"name": "06_translation_card", "box": (2775, 610, 3605, 1380)},
        {"name": "07_reader_rule_nodes", "box": (120, 1435, 3720, 1690)},
        {"name": "08_transition_badges", "box": (120, 1732, 3720, 1858)},
        {"name": "09_footer", "box": (120, 1910, 3720, 2076)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "prediction_hypothesis_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "prediction_hypothesis_STRICT_QA_contact_sheet.png")
