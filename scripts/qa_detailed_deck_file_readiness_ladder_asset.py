#!/usr/bin/env python3
"""Build visual QA crops for slide 54 file readiness ladder."""
...
ASSET_DIR = (
    ...
    / "assets"
    / "file_readiness_ladder"
)
IMAGE_PATH = ASSET_DIR / "file_readiness_ladder_turns_metadata_into_release_package_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"file_readiness_ladder_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"file_readiness_ladder_{name}.png"
    ...
    (target_dir / "file_readiness_ladder_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "03_full_ladder", "box": (120, 610, 3720, 1316)},
        {"name": "04_ladder_arrows", "box": (640, 785, 3190, 1042)},
        {"name": "05_study_row_checks", "box": (120, 1380, 2380, 1848)},
        {"name": "06_release_bundle", "box": (2460, 1380, 3720, 1848)},
    ...
    strict_specs = [
        ...
        {"name": "03_ladder_cards", "box": (160, 780, 3680, 1176)},
        {"name": "04_progress_track", "box": (165, 1200, 3680, 1302)},
        {"name": "05_dot_grids", "box": (160, 1510, 2340, 1775)},
        {"name": "06_release_requirements", "box": (2508, 1510, 3670, 1792)},
        {"name": "07_footer", "box": (120, 1910, 3720, 2076)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "file_readiness_ladder_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "file_readiness_ladder_STRICT_QA_contact_sheet.png")
