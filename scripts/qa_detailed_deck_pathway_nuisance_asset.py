#!/usr/bin/env python3
"""Build strict visual QA crops for the pathway nuisance-signal asset."""
...
SOURCE = ASSET_DIR / "pathway_features_reduce_selected_nuisance_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"pathway_nuisance_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"pathway_nuisance_{name}.png"
    ...
    (target_dir / "pathway_nuisance_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "02_feature_panel", "box": (130, 330, 1035, 1820)},
        {"name": "03_label_score_panel", "box": (1070, 330, 2545, 1820)},
        {"name": "04_detection_panel", "box": (2580, 330, 3710, 1820)},
        {"name": "05_footer", "box": (130, 1860, 3710, 2070)},
    ...
    strict_specs = [
        ...
        {"name": "03_gene_view_card", "box": (180, 555, 990, 1015)},
        {"name": "04_pathway_view_card", "box": (180, 1120, 990, 1580)},
        {"name": "05_reader_rule", "box": (180, 1515, 990, 1790)},
        {"name": "06_label_score_header_axis", "box": (1115, 360, 2495, 695)},
        {"name": "07_label_score_rows_top", "box": (1115, 655, 2495, 1125)},
        {"name": "08_label_score_rows_bottom", "box": (1115, 1085, 2495, 1655)},
        {"name": "09_label_score_legend", "box": (1115, 1600, 2495, 1790)},
        {"name": "10_detection_delta_chart", "box": (2625, 560, 3665, 1395)},
        {"name": "11_scope_readout", "box": (2625, 1415, 3665, 1790)},
        {"name": "12_footer_source", "box": (150, 1880, 3690, 2045)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "pathway_nuisance_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "pathway_nuisance_STRICT_QA_contact_sheet.png")
