#!/usr/bin/env python3
"""Build visual QA crops for slide 53 public bulk metadata alpha."""
...
ASSET_DIR = (
    ...
    / "assets"
    / "public_bulk_metadata_alpha"
)
IMAGE_PATH = ASSET_DIR / "public_bulk_metadata_alpha_makes_the_bulk_core_inspectable_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"public_bulk_metadata_alpha_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"public_bulk_metadata_alpha_{name}.png"
    ...
    (target_dir / "public_bulk_metadata_alpha_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "02_title_subtitle", "box": (120, 310, 3600, 520)},
        {"name": "03_indexed_panel", "box": (120, 610, 1260, 1464)},
        {"name": "04_coverage_panel", "box": (1335, 610, 2520, 1464)},
        {"name": "05_sequence_panel", "box": (2595, 610, 3720, 1464)},
        {"name": "06_bottom_object_path", "box": (120, 1530, 3720, 1848)},
    ...
    strict_specs = [
        ...
        {"name": "03_indexed_cards", "box": (160, 805, 1220, 1295)},
        {"name": "04_alpha_readout", "box": (160, 1325, 1220, 1435)},
        {"name": "05_coverage_dots", "box": (1388, 840, 2468, 1285)},
        {"name": "06_sequence_arrows", "box": (2628, 815, 3692, 1376)},
        {"name": "07_bottom_arrows", "box": (760, 1660, 3095, 1788)},
        {"name": "08_footer", "box": (120, 1910, 3720, 2076)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "public_bulk_metadata_alpha_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "public_bulk_metadata_alpha_STRICT_QA_contact_sheet.png")
