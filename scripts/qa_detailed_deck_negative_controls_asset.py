#!/usr/bin/env python3
"""Build strict visual QA crops for the negative-controls asset."""
...
SOURCE = ASSET_DIR / "negative_controls_anchor_readout_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"negative_controls_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"negative_controls_{name}.png"
    ...
    (target_dir / "negative_controls_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "02_control_families", "box": (130, 330, 1135, 1820)},
        {"name": "03_control_distribution", "box": (1150, 330, 2625, 1820)},
        {"name": "04_readout_rule", "box": (2640, 330, 3715, 1820)},
    ...
    strict_specs = [
        ...
        {"name": "03_control_family_top", "box": (180, 360, 1080, 980)},
        {"name": "04_control_family_bottom", "box": (180, 950, 1080, 1790)},
        {"name": "05_distribution_title_axis", "box": (1210, 360, 2545, 640)},
        {"name": "06_distribution_label_shuffle", "box": (1210, 550, 2545, 945)},
        {"name": "07_distribution_random_features", "box": (1210, 900, 2545, 1295)},
        {"name": "08_permutation_gate", "box": (1210, 1380, 2545, 1785)},
        {"name": "09_readout_signal_boundary", "box": (2700, 500, 3635, 1295)},
        {"name": "10_housekeeping_card", "box": (2700, 1280, 3635, 1790)},
        {"name": "11_footer_source", "box": (150, 1880, 3690, 2045)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "negative_controls_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "negative_controls_STRICT_QA_contact_sheet.png")
