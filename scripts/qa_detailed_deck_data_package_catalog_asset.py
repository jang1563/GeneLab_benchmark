#!/usr/bin/env python3
"""Build visual QA crops for slide 55 data package catalog."""
...
IMAGE_PATH = ASSET_DIR / "data_package_makes_the_metadata_alpha_portable_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"data_package_catalog_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"data_package_catalog_{name}.png"
    ...
    (target_dir / "data_package_catalog_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "03_catalog_descriptor", "box": (120, 610, 1260, 1464)},
        {"name": "04_metadata_shelves", "box": (1335, 610, 2520, 1464)},
        {"name": "05_reviewer_read_path", "box": (2595, 610, 3720, 1464)},
        {"name": "06_bottom_catalog_path", "box": (120, 1530, 3720, 1848)},
    ...
    strict_specs = [
        ...
        {"name": "03_descriptor_fields", "box": (165, 820, 1215, 1086)},
        {"name": "04_descriptor_counts", "box": (160, 1110, 1220, 1425)},
        {"name": "05_shelf_chips", "box": (1380, 820, 2475, 1365)},
        {"name": "06_read_path_arrows", "box": (2628, 812, 3692, 1384)},
        {"name": "07_bottom_arrows", "box": (760, 1660, 3095, 1788)},
        {"name": "08_footer", "box": (120, 1910, 3720, 2076)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "data_package_catalog_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "data_package_catalog_STRICT_QA_contact_sheet.png")
