#!/usr/bin/env python3
"""Run the v9 human organoid L2 logistic feature-effect pilot."""
...
PREDICTION_ROWS = [
    {
        "sample_id": "logistic_feature_effect_smoke_ground",
        "true_label": "Ground",
        "predicted_label": "Ground",
        "flight_probability": "0.05",
    },
    {
        "sample_id": "logistic_feature_effect_smoke_leo_or_iss",
    ...
    for key in sorted(value, key=lambda item: int(item)):
        ...
        rows.append(
            (
                ...
                _format_metric(row.get("overlap_fraction") if isinstance(row.get("overlap_fraction"), (int, float)) else None),
                _format_metric(row.get("expected_overlap") if isinstance(row.get("expected_overlap"), (int, float)) else None),
                _format_metric(row.get("enrichment") if isinstance(row.get("enrichment"), (int, float)) else None),
                _format_metric(row.get("hypergeometric_p_value_greater_equal") if isinstance(row.get("hypergeometric_p_value_greater_equal"), (int, float)) else None),
            )
        )
    return rows
...
def _write_note(
    *,
    ...
    lines = [
        "# Human Organoid L2 Logistic Feature-Effect Pilot",
        "",
        ...
        "training samples only. Feature effects are L2 logistic-regression",
        "coefficients in train-standardized gene space. They are not log2FC",
        ...
        "it is used only afterward for rank, sign, and top-k diagnostic scoring.",
        "",
        ...
        "| Metric | Value |",
        "|---|---:|",
        f"| `feature_effect_direction_match` | {_format_metric(_metric_value(evaluation, 'feature_effect_direction_match'))} |",
        f"| `feature_effect_rank_correlation` | {_format_metric(_metric_value(evaluation, 'feature_effect_rank_correlation'))} |",
        "",
        "## Top-K DE Overlap",
        "",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        default="v9/human_organoid/reports/logistic_feature_effect",
        help="Output directory for L2 logistic feature-effect report.",
    )
    parser.add_argument(
        "--top-variable-genes",
        type=int,
