#!/usr/bin/env python3
"""Build strict visual QA crops for the foundation-model concept asset."""
...
SOURCE = ASSET_DIR / "what_a_foundation_model_adds_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"foundation_model_adds_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"foundation_model_adds_{name}.png"
    ...
    (target_dir / "foundation_model_adds_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "02_fixed_contract", "box": (120, 330, 1090, 1840)},
        {"name": "03_added_encoder", "box": (1085, 330, 2635, 1840)},
        {"name": "04_pretraining_levers", "box": (2630, 330, 3730, 1840)},
    ...
    strict_specs = [
        ...
        {"name": "03_contract_top", "box": (150, 355, 1045, 880)},
        {"name": "04_contract_bottom", "box": (150, 840, 1045, 1810)},
        {"name": "05_pretraining_routes", "box": (1135, 355, 2580, 750)},
        {"name": "06_encoder_graph", "box": (1135, 725, 2580, 1220)},
        {"name": "07_adapter_gate", "box": (1135, 1180, 2580, 1810)},
        {"name": "08_levers_top", "box": (2675, 355, 3680, 1010)},
        {"name": "09_levers_bottom", "box": (2675, 970, 3680, 1510)},
        {"name": "10_reading_rule", "box": (2675, 1460, 3680, 1810)},
        {"name": "11_footer_source", "box": (130, 1860, 3710, 2130)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "foundation_model_adds_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "foundation_model_adds_STRICT_QA_contact_sheet.png")
