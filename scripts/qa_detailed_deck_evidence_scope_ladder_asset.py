#!/usr/bin/env python3
"""Build strict visual QA crops for the evidence scope ladder asset."""
...
SOURCE = ASSET_DIR / "evidence_scope_ladder_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"evidence_scope_ladder_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"evidence_scope_ladder_{name}.png"
    ...
    (target_dir / "evidence_scope_ladder_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "02_reader_contract", "box": (130, 320, 1035, 1830)},
        {"name": "03_evidence_ladder", "box": (1045, 330, 2495, 1830)},
        {"name": "04_unlock_panel", "box": (2505, 320, 3720, 1830)},
    ...
    strict_specs = [
        ...
        {"name": "03_reader_contract_top", "box": (180, 390, 975, 700)},
        {"name": "04_reader_contract_rows", "box": (180, 600, 975, 1320)},
        {"name": "05_reader_contract_readout", "box": (180, 1610, 975, 1780)},
        {"name": "06_ladder_layers_top", "box": (1060, 335, 2485, 970)},
        {"name": "07_ladder_layers_bottom", "box": (1060, 990, 2485, 1790)},
        {"name": "08_unlock_rows", "box": (2585, 560, 3640, 1235)},
        {"name": "09_unlock_reading_move", "box": (2585, 1555, 3640, 1780)},
        {"name": "10_footer", "box": (150, 1880, 3690, 2045)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "evidence_scope_ladder_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "evidence_scope_ladder_STRICT_QA_contact_sheet.png")
