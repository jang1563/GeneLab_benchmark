#!/usr/bin/env python3
"""Build visual QA crops for slide 40 mouse-human transfer asset."""
...
ASSET_DIR = (
    ...
    / "assets"
    / "mouse_human_transfer"
)
SOURCE = ASSET_DIR / "pathways_carry_mouse_to_human_transfer_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"mouse_human_transfer_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"mouse_human_transfer_{name}.png"
    ...
    (target_dir / "mouse_human_transfer_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "02_title_subtitle", "box": (120, 310, 3100, 545)},
        {"name": "03_gene_panel", "box": (120, 610, 1280, 1658)},
        {"name": "04_pathway_panel", "box": (1340, 610, 2500, 1658)},
        {"name": "05_target_panel", "box": (2560, 610, 3720, 1658)},
        {"name": "06_reader_rule", "box": (120, 1694, 3720, 1848)},
    ...
    strict_specs = [
        ...
        {"name": "02_header_title_badges", "box": (100, 50, 3740, 540)},
        {"name": "03_gene_fold_bars", "box": (155, 895, 1245, 1120)},
        {"name": "04_gene_auroc_bars", "box": (155, 1160, 1245, 1515)},
        {"name": "05_pathway_axis_bars", "box": (1385, 900, 2460, 1345)},
        {"name": "06_pathway_nes_pills", "box": (1385, 1350, 2460, 1515)},
        {"name": "07_cfrna_panel_dots", "box": (2605, 900, 3685, 1055)},
        {"name": "08_target_tiers", "box": (2605, 1060, 3685, 1275)},
        {"name": "09_tf_lane", "box": (2605, 1350, 3685, 1526)},
        {"name": "10_reader_flow_arrows", "box": (150, 1700, 3690, 1838)},
        {"name": "11_footer", "box": (120, 1910, 3720, 2076)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "mouse_human_transfer_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "mouse_human_transfer_STRICT_QA_contact_sheet.png")
