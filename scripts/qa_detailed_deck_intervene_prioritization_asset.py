#!/usr/bin/env python3
"""Build visual QA crops for slide 47 INTERVENE prioritization asset."""
...
ASSET_DIR = (
    ...
    / "assets"
    / "intervene_prioritization"
)
SOURCE = ASSET_DIR / "perturbation_hits_prioritize_followup_axes_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"intervene_prioritization_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"intervene_prioritization_{name}.png"
    ...
    (target_dir / "intervene_prioritization_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "04_pareto_panel", "box": (1265, 610, 2575, 1462)},
        {"name": "05_target_axis_panel", "box": (2630, 610, 3720, 1462)},
        {"name": "06_intervene_flow_strip", "box": (120, 1530, 3720, 1848)},
    ...
    strict_specs = [
        ...
        {"name": "03_method_signature_logic", "box": (120, 720, 1210, 1070)},
        {"name": "04_method_steps_rule", "box": (120, 1050, 1210, 1462)},
        {"name": "05_pareto_metric_cards", "box": (1265, 760, 2575, 940)},
        {"name": "06_pareto_scatter_labels", "box": (1265, 930, 2575, 1338)},
        {"name": "07_pareto_callout", "box": (1265, 1324, 2575, 1462)},
        {"name": "08_target_axis_rows", "box": (2630, 820, 3720, 1188)},
        {"name": "09_crispr_grid_and_card", "box": (2630, 1190, 3720, 1462)},
        {"name": "10_flow_nodes", "box": (120, 1530, 3720, 1848)},
        {"name": "11_footer", "box": (120, 1910, 3720, 2076)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "intervene_prioritization_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "intervene_prioritization_STRICT_QA_contact_sheet.png")
