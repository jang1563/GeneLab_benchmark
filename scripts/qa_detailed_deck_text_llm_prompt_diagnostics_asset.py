#!/usr/bin/env python3
"""Build strict visual QA crops for the text-LLM prompt diagnostic asset."""
...
SOURCE = ASSET_DIR / "text_llm_prompt_diagnostics_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"text_llm_prompt_diagnostics_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"text_llm_prompt_diagnostics_{name}.png"
    ...
    (target_dir / "text_llm_prompt_diagnostics_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "02_prompt_panel", "box": (120, 330, 1220, 1840)},
        {"name": "03_pipeline_panel", "box": (1210, 330, 2570, 1840)},
        {"name": "04_readout_panel", "box": (2560, 330, 3730, 1840)},
    ...
    strict_specs = [
        ...
        {"name": "03_prompt_top", "box": (170, 360, 1180, 880)},
        {"name": "04_prompt_middle", "box": (170, 820, 1180, 1375)},
        {"name": "05_prompt_bottom", "box": (170, 1325, 1180, 1805)},
        {"name": "06_pipeline_top_steps", "box": (1270, 360, 2515, 885)},
        {"name": "07_pipeline_middle_steps", "box": (1270, 815, 2515, 1375)},
        {"name": "08_pipeline_bottom_rule", "box": (1270, 1315, 2515, 1805)},
        {"name": "09_provider_chart", "box": (2625, 500, 3665, 1010)},
        {"name": "10_readout_cards_top", "box": (2625, 1000, 3665, 1390)},
        {"name": "11_readout_cards_bottom", "box": (2625, 1365, 3665, 1805)},
        {"name": "12_footer_source", "box": (130, 1860, 3710, 2130)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "text_llm_prompt_diagnostics_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "text_llm_prompt_diagnostics_STRICT_QA_contact_sheet.png")
