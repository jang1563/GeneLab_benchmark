#!/usr/bin/env python3
"""Build visual QA crops for slide 39 target/biomarker asset."""
...
SOURCE = ASSET_DIR / "target_biomarker_layers_are_triage_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"target_biomarker_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"target_biomarker_{name}.png"
    ...
    (target_dir / "target_biomarker_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "03_metabolic_panel", "box": (120, 610, 1325, 1685)},
        {"name": "04_target_panel", "box": (1335, 610, 2540, 1685)},
        {"name": "05_biomarker_panel", "box": (2550, 610, 3755, 1685)},
    ...
    strict_specs = [
        ...
        {"name": "02_title_subtitle", "box": (120, 350, 3120, 625)},
        {"name": "03_metabolic_coverage", "box": (190, 1025, 1260, 1395)},
        {"name": "04_metabolic_subsystems", "box": (190, 1410, 1260, 1605)},
        {"name": "05_target_funnel", "box": (1400, 1025, 2485, 1395)},
        {"name": "06_target_tiers", "box": (1400, 1410, 2485, 1605)},
        {"name": "07_panel_auroc", "box": (2615, 1025, 3705, 1395)},
        {"name": "08_panel_matrix", "box": (2615, 1385, 3705, 1635)},
        {"name": "09_reader_rule_arrows", "box": (150, 1695, 3690, 1828)},
        {"name": "10_footer", "box": (120, 1865, 3720, 2095)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "target_biomarker_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "target_biomarker_STRICT_QA_contact_sheet.png")
