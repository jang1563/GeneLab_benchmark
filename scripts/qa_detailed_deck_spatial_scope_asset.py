#!/usr/bin/env python3
"""Build visual QA crops for slide 36 spatial scope asset."""
...
SOURCE = ASSET_DIR / "spatial_weak_signal_cases_define_scope_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"spatial_scope_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"spatial_scope_{name}.png"
    ...
    (target_dir / "spatial_scope_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "02_title_subtitle", "box": (120, 350, 2850, 625)},
        {"name": "03_input_panel", "box": (120, 610, 1325, 1655)},
        {"name": "04_readout_panel", "box": (1335, 610, 2540, 1655)},
        {"name": "05_structure_panel", "box": (2550, 610, 3755, 1655)},
    ...
    strict_specs = [
        ...
        {"name": "02_title_subtitle", "box": (120, 350, 2850, 625)},
        {"name": "03_input_stats", "box": (170, 745, 1260, 990)},
        {"name": "04_section_tiles", "box": (170, 1110, 1265, 1585)},
        {"name": "05_quality_cue", "box": (170, 1510, 1265, 1625)},
        {"name": "06_readout_stats", "box": (1385, 745, 2480, 990)},
        {"name": "07_auroc_bars", "box": (1385, 1000, 2505, 1490)},
        {"name": "08_readout_status", "box": (1385, 1450, 2505, 1625)},
        {"name": "09_svg_dotplot", "box": (2605, 1015, 3705, 1305)},
        {"name": "10_overlap_pc1", "box": (2605, 1315, 3705, 1625)},
        {"name": "11_reader_rule_arrows", "box": (150, 1695, 3690, 1828)},
        {"name": "12_footer", "box": (120, 1865, 3720, 2095)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "spatial_scope_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "spatial_scope_STRICT_QA_contact_sheet.png")
