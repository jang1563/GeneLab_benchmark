#!/usr/bin/env python3
"""Build visual QA crops for slide 48 CAUSAL map asset."""
...
SOURCE = ASSET_DIR / "causal_maps_connect_stressor_axes_to_tissue_readouts_premium.png"
...
def write_contact_sheet(crops: list[dict[str, object]], directory: Path, sheet_name: str) -> Path:
    ...
    for crop in crops:
        name = str(crop["name"])
        image = Image.open(directory / f"causal_map_{name}.png")
...
def crop_set(target_dir: Path, specs: list[dict[str, object]], sheet_name: str) -> Path:
    ...
    for spec in specs:
        ...
        out = target_dir / f"causal_map_{name}.png"
    ...
    (target_dir / "causal_map_crop_manifest.json").write_text(
...
def main() -> None:
    qa_specs = [
        ...
        {"name": "04_edge_network", "box": (1265, 610, 2575, 1462)},
        {"name": "05_stability_panel", "box": (2630, 610, 3720, 1462)},
        {"name": "06_causal_flow_strip", "box": (120, 1530, 3720, 1848)},
    ...
    strict_specs = [
        ...
        {"name": "03_method_cards_flow", "box": (120, 800, 1210, 1225)},
        {"name": "04_method_icp_readout", "box": (120, 1210, 1210, 1462)},
        {"name": "05_network_nodes_edges", "box": (1265, 780, 2575, 1330)},
        {"name": "06_network_callout", "box": (1265, 1338, 2575, 1462)},
        {"name": "07_stability_bars", "box": (2630, 750, 3720, 1295)},
        {"name": "08_stability_cards", "box": (2630, 1235, 3720, 1462)},
        {"name": "09_flow_nodes", "box": (120, 1530, 3720, 1848)},
        {"name": "10_footer", "box": (120, 1910, 3720, 2076)},
    ]
    qa_sheet = crop_set(QA_DIR, qa_specs, "causal_map_qa_crop_contact_sheet.png")
    strict_sheet = crop_set(STRICT_DIR, strict_specs, "causal_map_STRICT_QA_contact_sheet.png")
